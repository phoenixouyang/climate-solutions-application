## About The Project
This is a web application that uses data from [Project Drawdown](https://drawdown.org/) to display information about their climate solution projects to share stories of innovation and hope! Please check out their website or the links on this page to learn more about the great work they're doing.

This page is written using Javascript, Ejs, and TailwindCSS, and features dynamic route handling, APIs from [DummyJSON](https://dummyjson.com/) and page error handling.

### Built With
[![Express.js]][Express.js-url]
[![Tailwind]][Tailwind-url]
[![DaisyUI]][DaisyUI-url]

<img src="public/climate-solutions.png" alt="screenshot of climate solutions homepage" width="800">

## Getting Started
To check out the page, you can access it from the deployed link [here](https://climate-solutions-drawdown.vercel.app/).

## Contact

Phoenix Ouyang - [LinkedIn](https://www.linkedin.com/in/phoenixouyang/) - [GitHub](https://github.com/phoenixouyang)

<!-- MARKDOWN LINKS & IMAGES -->
[Express.js]: https://img.shields.io/badge/espress.js-black?style=for-the-badge&logo=express&link=https%3A%2F%2Fexpressjs.com%2F
[Express.js-url]: https://expressjs.com/
[Tailwind]: https://img.shields.io/badge/tailwind-black?style=for-the-badge&logo=tailwindcss&link=https%3A%2F%2Ftailwindcss.com%2F
[Tailwind-url]: https://tailwindcss.com/
[DaisyUI]: https://img.shields.io/badge/daisyui-grey?style=for-the-badge&logo=daisyui&link=https%3A%2F%2Fdaisyui.com%2F%3Flang%3Den
[DaisyUI-url]: https://daisyui.com/?lang=en
